...
class Dogpatch extends Curl {
    ...
    public function assertBodyContains($asserted, $id, $onNotEqualVarExport = false, $not = false) {
        if (empty($this->body)) {
            $this->body = substr($this->response, $this->getCurlInfo(CURLINFO_HEADER_SIZE));
        }
        ...
        foreach ($body as $item) {
            foreach ($asserted as $assertedItem)
                if (!in_array($assertedItem, $item))
                    continue 2;
            if ($not) {
                $error = 1;
                break;
        ...
        if ($not && !isset($error))
            return $this;
        $errorMessage = "Response body ".($not ? "contains" : "does not contain")." asserted item.";
        if ($onNotEqualVarExport) {
            $errorMessage .= "\n\n--------------- ASSERTED BODY ---------------\n" . var_export($asserted, true) .
                            "\n\n--------------- RESPONSE BODY ---------------\n" . var_export($body, true) . "\n\n";
        }
